...
{
	static gltexture_t temp_texture;
	GLuint texnum;
	...
	/* Upload RGBA data */
	glTexImage2D_fp(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, data);

	/* Return a temporary gltexture_t structure */
	temp_texture.texnum = texnum;
	temp_texture.width = width;
	temp_texture.height = height;
	temp_texture.flags = flags;
	strcpy(temp_texture.identifier, name);
