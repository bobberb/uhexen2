...
extern cvar_t	gl_max_size; /* defined in gl_rmisc.c */
...
unsigned int d_8to24table_opaque[256];  // Hexen2: index 255 is opaque black for UI/non-transparent textures
...
static void TexMgr_SetFilterModes(gltexture_t *glt)
{
	...
	{
		glTexParameterf_fp(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, modes[glmode_idx].magfilter);
		glTexParameterf_fp(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, modes[glmode_idx].minfilter);
		glTexParameterf_fp(GL_TEXTURE_2D, GL_TEXTURE_MAX_ANISOTROPY_EXT, gl_texture_anisotropy.value);
	}
	else
	{
		glTexParameterf_fp(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, modes[glmode_idx].magfilter);
		glTexParameterf_fp(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, modes[glmode_idx].magfilter);
	}
}
...
static void TexMgr_TextureMode_f(cvar_t *var)
{
	...
	for (i = 0; i < NUM_GLMODES; i++)
	{
		if (!strcmp(modes[i].name, gl_texturemode.string))
		{
			if (glmode_idx != i)
			{
				glmode_idx = i;
	...
	Cvar_SetQuick(&gl_texturemode, modes[glmode_idx].name);
}
...
static void TexMgr_Imagedump_f(void)
{
	...
	//create directory
	q_snprintf(dirname, sizeof(dirname), "%s/imagedump", fs_gamedir);
	Sys_mkdir(dirname, false);
...
void TexMgr_LoadPalette(void)
{
	...
	// Index 255 is the transparency color (alpha = 0)
	...
	//opaque palette for UI/non-transparent textures - like standard palette but index 255 is opaque
	memcpy(d_8to24table_opaque, d_8to24table, 256 * 4);
	// Make index 255 opaque black (for UI elements that use index 255 as black, not transparent)
	((byte *)&d_8to24table_opaque[255])[0] = 0;
	((byte *)&d_8to24table_opaque[255])[1] = 0;
	((byte *)&d_8to24table_opaque[255])[2] = 0;
	((byte *)&d_8to24table_opaque[255])[3] = 255;
...
static void TexMgr_LoadImage8(gltexture_t *glt, byte *data)
{
	...
	// Sprites (.spr files) must use transparent palette for their rectangular bounds
	// Restore TEXPREF_ALPHA for sprites even if they have no index 255 pixels
	if (glt->source_file[0] && strstr(glt->source_file, ".spr"))
	{
		glt->flags |= TEXPREF_ALPHA;
	}
	...
	if (glt->flags & TEXPREF_FULLBRIGHT)
	{
		if (glt->flags & TEXPREF_ALPHA)
			...
			usepal = d_8to24table_fbright;  // Has index 255 as transparent
		padbyte = 255;
	}
	else if (glt->flags & TEXPREF_NOBRIGHT && gl_fullbrights.value)
	{
		if (glt->flags & TEXPREF_ALPHA)
			...
			usepal = d_8to24table_nobright;  // Has index 255 as transparent
		padbyte = 255;
	}
	...
	{
		// For non-transparent textures, use opaque palette where index 255 is solid black
		// For transparent textures, use standard palette where index 255 is transparent
		if (glt->flags & TEXPREF_ALPHA)
			usepal = d_8to24table;
		else
			usepal = d_8to24table_opaque;
		padbyte = 255;
	}
	...
	byte *data, const char *source_file, src_offset_t source_offset, unsigned flags)
{
	...
	// copy data
	glt->owner = (struct qmodel_t *)owner;
...
static void GL_DeleteTexture(gltexture_t *texture)
{
	glDeleteTextures_fp(1, &texture->texnum);
